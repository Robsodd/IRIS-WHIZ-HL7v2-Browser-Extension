...
chrome.storage.local.get(['instances', 'settings'], function(stored) {
    storedData = stored;
    instances = stored.instances;
    console.log("instances", instances);
    let optionsPageBtn = document.getElementById("optionsPageButton");
    
    // Safely check if instances exists before checking length
    if (!instances || instances.length == 0) {
        optionsPageBtn.style.backgroundColor = "orange";
        optionsPageBtn.innerText = "Options Page: Alert: Some features will not work without instances set up here.";
    } else {
        // no alert
    }
})
...
// Read version directly from manifest.json to stay Web Store compliant
let versionBtn = document.getElementById("versionBtn");
const currentVersion = chrome.runtime.getManifest().version;

versionBtn.innerText = `v${currentVersion}`;
versionBtn.style.display = "block";

versionBtn.addEventListener("click", function() {
    console.log("Clicked version");
    // Open the GitHub releases page for users to see what's new
    window.open("https://github.com/Robsodd/IRIS-WHIZ-HL7v2-Browser-Extension/releases");
});
